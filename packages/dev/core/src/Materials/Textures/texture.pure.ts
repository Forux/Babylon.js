...
import { type CubeTexture } from "../../Materials/Textures/cubeTexture.pure";
import { type MirrorTexture } from "../../Materials/Textures/mirrorTexture.pure";
...
export class Texture extends BaseTexture {
    ...
    public override delayLoad(): void {
        ...
        //> VRNET
        const delayedOnLoad = this._delayedOnLoad;
        const delayedOnError = this._delayedOnError;
        //< VRNET

        this._texture = this._getFromCache(url, this._noMipmap, this.samplingMode, this._invertY, this._useSRGBBuffer, this.isCube);

        if (!this._texture) {
            this._texture = scene.getEngine().createTexture(
                url,
                this._noMipmap,
                this._invertY,
                scene,
        ...
        } else if (delayedOnLoad) {
            if (this._texture.isReady) {
                TimingTools.SetImmediate(() => delayedOnLoad(this._texture!));
            } else {
                const loadObserver = this._texture.onLoadedObservable.add(() => {
                    delayedOnLoad(this._texture!);
                });

                if (delayedOnError) {
                    this._texture.onErrorObservable.add(() => {
                        delayedOnError();
                        this._texture?.onLoadedObservable.remove(loadObserver);
                    });
                }
